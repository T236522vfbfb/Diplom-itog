  ...
  async run() {
    try {
      ...
      for (let i = 0; i < MAX_COACHES; i++) {
        const classIndex = rand(0, Object.keys(classTypes).length - 1), 
              ...
              attributes = {
                ...
                price: isFirstClass ? this.generatePrice(classInfo) : null,
                top_price: this.generatePrice(classInfo),
                bottom_price: this.generatePrice(classInfo),
                side_price: haveSideSeats ? this.generatePrice(classInfo) : null,
                linens_price: !isFourthClass ? rand(50, 300) : null,
