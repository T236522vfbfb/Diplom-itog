...
const getBoolProps = compose(
    mapObjIndexed(val => val === 'true'),
    pick(boolProps)
);

const isNotNil = compose(not, isNil);
const omitEmptyBoolProps = compose(pickBy(isNotNil), getBoolProps);
...
export const toRoute = item => {
    ...
    const { price_info: priceInfo = {} } = item;
...
export const toAvailableSeats = boughtSeats => coach => {
    const eqCoach = eq(coach._id),
          boughtCoachSeats = boughtSeats.filter(({ coach }) => eqCoach(coach._id)), 
          seats = range(0, coach.available_seats)
              .map(position => {
                  const index = inc(position),
                        eqIndex = eq(index),
                        seat = boughtCoachSeats.find(({ seatIndex }) => eqIndex(seatIndex)), 
...
export const toSeats = (route, train, attributes) => {
   ...
   return coaches
       ...
       .map(toAvailableSeats(boughtSeats));
};